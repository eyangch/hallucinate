...
def train():
    ...
    try:
        #raise Exception
        model = models.load_ae("/cartpole/model_ae_8_interp.torch")
        print("loaded model")
    ...
    def loss_fn(recon_x, x): 
        return F.mse_loss(recon_x, x)*192*192

    def loss_fn_special(recon_x, x): 
        # since so much of the image is white, weight nonwhite parts more
        # without this modification, AE would converge on outputting fully white
        flattened_x = torch.flatten(x)
        flattened_recon_x = torch.flatten(recon_x)
        white_loc = flattened_x > 0.99
        nonwhite_loc = flattened_x <= 0.99
        return (
            F.mse_loss(flattened_recon_x[white_loc], flattened_x[white_loc]) * 0.05 * torch.count_nonzero(white_loc) / 3
            + F.mse_loss(flattened_recon_x[nonwhite_loc], flattened_x[nonwhite_loc]) * torch.count_nonzero(nonwhite_loc) / 3
        )
        #return F.mse_loss(recon_x, x)*192*192
    ...
    def ae_loss_fn(recon_x, x):
        recon_loss = loss_fn_special(recon_x, x)
    ...
    def generate_data(batch_size, k):
        ...
        batched_data = tensor_data[:num_batches * batch_size].view(num_batches, batch_size, 3, 192, 192)
        torch.manual_seed(8765)
    ...
    def train_ae_epoch(train_dataloader):
        ...
        for idx, images in enumerate(train_dataloader):
            ...
            interp_loss = batch_interpretable_loss(encoded_images, p=2) * 100
    ...
    def train(batch_size=64):
        ...
        while data_generated < 1:
            ...
            t_start = time.time()
            train_dataloader, test_dataloader, _ = generate_data(batch_size, 80)
            ...
            while True:
                ...
                if epoch % 5 == 0:
                    torch.save(model.state_dict(), "/cartpole/model_ae_8_interp.torch")
                    vol.commit()
